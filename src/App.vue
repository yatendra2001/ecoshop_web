<template>
    ...
    <div class="relative flex flex-col" v-else>
        <Header class="sticky top-0 z-50" v-if="route.name !== 'Login' && !route.path.includes('/admin')"></Header>
        <div class="flex-grow"><router-view /></div>

        <Footer class="mt-20" v-if="route.name !== 'Login' && !route.path.includes('/admin')"></Footer>
    </div>
</template>
<script>
